...
async function addTodayTrackingDataFixed() {
  let connection;
  
  try {
    console.log('📅 Adding tracking data for today (fixed timezone)...\n');
    
    connection = await mysql.createConnection(dbConfig);
    console.log('✅ Connected to database');

    // Get today's date in the correct format for database
    const today = new Date();
    const todayString = today.toISOString().split('T')[0];
    console.log('📅 Today\'s date (ISO):', todayString);
    ...
    const [existingMood] = await connection.execute(
      'SELECT COUNT(*) as count FROM mood_tracking WHERE user_id = ? AND DATE(tracking_date) = ?',
      [userId, todayString]
    );
    const [existingWater] = await connection.execute(
      'SELECT COUNT(*) as count FROM water_tracking WHERE user_id = ? AND DATE(tracking_date) = ?',
      [userId, todayString]
    );
    const [existingFitness] = await connection.execute(
      'SELECT COUNT(*) as count FROM fitness_tracking WHERE user_id = ? AND DATE(tracking_date) = ?',
      [userId, todayString]
    );
    const [existingSleep] = await connection.execute(
      'SELECT COUNT(*) as count FROM sleep_tracking WHERE user_id = ? AND DATE(sleep_date) = ?',
      [userId, todayString]
    );
    ...
    if (existingMood[0].count === 0) {
      ...
      try {
        await connection.execute(
          ...
          [userId, 'happy', 8, 'low', 'high', 'good', todayString, 'Great day! Feeling energetic and productive']
        );
      ...
      for (const water of waterEntries) {
        try {
          await connection.execute(
            ...
            [userId, water.amount_ml, todayString, water.time, water.notes]
          );
      ...
      for (const fitness of fitnessActivities) {
        try {
          await connection.execute(
            ...
            [userId, fitness.activity_type, fitness.activity_name, fitness.duration_minutes, fitness.calories_burned, fitness.distance_km, fitness.intensity, todayString, fitness.time, fitness.notes]
          );
    ...
    if (existingSleep[0].count === 0) {
      ...
      try {
        await connection.execute(
          ...
          [userId, todayString, '22:30:00', '06:00:00', 450, 'excellent', 'Perfect sleep! Woke up feeling refreshed']
        );
      ...
      for (const meal of mealEntries) {
        try {
          await connection.execute(
            ...
            [userId, meal.meal_type, `${todayString} ${meal.time}`, meal.notes]
          );
