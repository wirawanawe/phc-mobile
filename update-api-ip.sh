...
if curl -s http://localhost:5432/health > /dev/null; then
    echo "✅ Backend is running on port 5432"
else
...
case $choice in
    1)
        ...
        echo "✅ Android emulator will use: http://10.0.2.2:5432/api"
        echo "✅ No changes needed - already configured automatically"
        ;;
    2)
        echo "🍎 Configuring for iOS Simulator..."
        echo "✅ iOS simulator will use: http://localhost:5432/api"
        echo "✅ No changes needed - already configured automatically"
        ;;
    3)
        ...
        echo "   // return \"http://$CURRENT_IP:5432/api\";"
        echo "3. Uncomment it and comment out the default:"
        echo "   return \"http://$CURRENT_IP:5432/api\";"
        echo "   // return \"http://localhost:5432/api\";"
        echo ""
        echo "🔗 Your device should connect to: http://$CURRENT_IP:5432/api"
        ;;
    4)
        ...
        if curl -s http://localhost:5432/health > /dev/null; then
            echo "✅ localhost:5432 - OK"
        else
            echo "❌ localhost:5432 - Failed"
        fi
        
        echo ""
        echo "Testing your IP..."
        if curl -s http://$CURRENT_IP:5432/health > /dev/null; then
            echo "✅ $CURRENT_IP:5432 - OK"
        else
            echo "❌ $CURRENT_IP:5432 - Failed"
        fi
        
        echo ""
        echo "Testing Android emulator IP..."
        if curl -s http://10.0.2.2:5432/health > /dev/null; then
            echo "✅ 10.0.2.2:5432 - OK"
        else
            echo "❌ 10.0.2.2:5432 - Failed (expected for non-emulator)"
        fi
        ;;
    5)
        echo "👋 Goodbye!"
